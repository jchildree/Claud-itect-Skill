...
before using the other two.  `/adr` allows the user to create/review/update an Architectural Design Record, which allows Claude Code to stay on task and efficiently utilize tokens. This creates a requirements in Markdown, that Claude digests well. `/phase` updates the user on the current phase status and ADRs.
